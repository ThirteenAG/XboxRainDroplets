...
kiero::Status::Enum kiero::init(RenderType::Enum _renderType)
{
	...
	{
		if (_renderType >= RenderType::D3D8 && _renderType <= RenderType::D3D12)
		{
			...
			if (_renderType == RenderType::D3D8)
			{
				...
				if ((Direct3DCreate8 = ::GetProcAddress(libD3D8, "Direct3DCreate8")) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
				...
				D3D8_LPDIRECT3D8 direct3D8;
				if ((direct3D8 = ((D3D8_LPDIRECT3D8(__stdcall*)(uint32_t))(Direct3DCreate8))(220)) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return Status::UnknownError;
				...
				if (direct3D8->CreateDevice(D3DADAPTER_DEFAULT, D3D8_D3DDEVTYPE_REF, window, DWORD(D3D8_D3DCREATE_SOFTWARE_VERTEXPROCESSING | D3D8_D3DCREATE_DISABLE_DRIVER_MANAGEMENT), &params, (void**)&device) < 0)
				{
					direct3D8->Release();
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
			...
			else if (_renderType == RenderType::D3D9)
			{
				...
				//	if (SUCCEEDED(((HRESULT(__stdcall*)(UINT, IDirect3D9Ex**))(Direct3DCreate9Ex))(D3D_SDK_VERSION, &direct3D9Ex)))
				//	{
				//		if (SUCCEEDED(direct3D9Ex->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_NULLREF, window, D3DCREATE_SOFTWARE_VERTEXPROCESSING | D3DCREATE_DISABLE_DRIVER_MANAGEMENT, &params, &device)))
				//		{
				//			::memcpy(g_methodsTable, *(uintptr_t**)device, IDirect3DDevice9VTBL::GetDisplayModeEx * sizeof(uintptr_t));
				//		}
				//	}
				//}
...
kiero::Status::Enum kiero::bind(uint16_t _index, void** _original, void* _function)
{
	...
	{
#if KIERO_USE_MINHOOK
		void* target = (void*)g_methodsTable[g_renderType][_index];
...
void kiero::unbind(uint16_t _index)
{
	...
	{
#if KIERO_USE_MINHOOK
		MH_DisableHook((void*)g_methodsTable[g_renderType][_index]);
#endif
	}
}
