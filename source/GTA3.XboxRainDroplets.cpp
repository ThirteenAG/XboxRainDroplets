...
// ---------------------------------------------------------------------------
// GTA III 1.0. What the drops answer to is the original effect, see the Neo
// water drops of skygfx: the same functions of the game are hooked and the
...
// ---------------------------------------------------------------------------
// The game, the addresses are the ones of GTA III 1.0
// ---------------------------------------------------------------------------

float& CTimer__ms_fTimeStep = *(float*)0x8E2CB4;	// CTimer::ms_fTimeStep
...
float& CWeather__Rain = *(float*)0x8E2BFC;		// CWeather::Rain
bool& CCutsceneMgr__ms_running = *(bool*)0x95CCF5;	// CCutsceneMgr::ms_running
uint8_t* TheCamera = (uint8_t*)0x6FACF8;		// TheCamera
uint8_t* Scene = (uint8_t*)0x726768;			// Scene, its camera is the second member
void** ppRwD3DDevice = (void**)0x662EF0;		// RwD3DDevice

static auto CCullZones__CamNoRain = (bool(__cdecl*)())0x525CE0;
static auto CCullZones__PlayerNoRain = (bool(__cdecl*)())0x525D00;
static auto FindPlayerVehicle = (bool(__cdecl*)())0x4A10C0;

struct CPad;

static auto CPad__GetPad = (CPad * (__cdecl*)(int))0x492F60;
static bool CPad__GetLookBehindForCar(CPad* pad) { return ((bool(__thiscall*)(CPad*))0x4932F0)(pad); }
static bool CPad__GetLookLeft(CPad* pad) { return ((bool(__thiscall*)(CPad*))0x493290)(pad); }
static bool CPad__GetLookRight(CPad* pad) { return ((bool(__thiscall*)(CPad*))0x4932C0)(pad); }
...
static AudioHydrant* audioHydrants = (AudioHydrant*)0x62DAAC;

// CParticleObject is a CPlaceable_III, its matrix sits behind the vtable of the
// object and the position of that matrix is the position of the object
constexpr auto PLACEABLE_POSITION = 0x34;
...
static short GetCamMode()
{
    ...
    const uint8_t* cam = TheCamera + 0x1A4 + active * 0x1A4;
...
static float GetParticleDistance(int id)
{
    switch (id)
    {
        case PARTICLE_WHEEL_WATER:		return 5.0f;
...
static void RenderEffectsHook()
{
    ...
    RegisterWidescreenResetCallback(L"GTA3.WidescreenFix.asi");
...
void Init()
{
    ...
    renderEffects.fun = injector::MakeCALL(0x48E603, RenderEffectsHook, true).get();

    static void(*resetHooks[5])() = { ResetHook1, ResetHook2, ResetHook3, ResetHook4, ResetHook5 };
    static const uintptr_t resetCalls[5] = { 0x48C1AB, 0x48C530, 0x42155D, 0x42177A, 0x421926 };	// CGame::Initialise, CGame::ReInitGameObjectVariables, CGameLogic::Update
    ...
    splashBreak = (uintptr_t)injector::GetBranchDestination(0x4BC7D0).get<void>();
    injector::MakeRelativeOffset(0x4BC7D0 + 1, (void*)SplashHook, 4, true);

    // the particles of the game, the ones commented out in the original are
    // the rain, which the effect already handles on its own
    addParticle1.fun = injector::MakeCALL(0x004D00DC, AddParticleHook1, true).get();	// blood spurt
    injector::MakeCALL(0x004E78D1, AddParticleHook1, true);					// blood
    injector::MakeCALL(0x004E946D, AddParticleHook1, true);					// blood
    injector::MakeCALL(0x004EB024, AddParticleHook1, true);					// blood small
    injector::MakeCALL(0x004EB564, AddParticleHook1, true);					// blood small
    injector::MakeCALL(0x0050E3AF, AddParticleHook1, true);					// rain splash
    injector::MakeCALL(0x0050E4E7, AddParticleHook1, true);					// rain splash
    injector::MakeCALL(0x00549815, AddParticleHook1, true);					// boat splash
    injector::MakeCALL(0x005588AA, AddParticleHook1, true);					// blood small
    injector::MakeCALL(0x0055CF07, AddParticleHook1, true);					// blood
    injector::MakeCALL(0x0055CF2E, AddParticleHook1, true);					// blood
    injector::MakeCALL(0x0055CF55, AddParticleHook1, true);					// blood
    injector::MakeCALL(0x0055CFE2, AddParticleHook1, true);					// blood
    injector::MakeCALL(0x0055D064, AddParticleHook1, true);					// blood
    injector::MakeCALL(0x0055FDE8, AddParticleHook1, true);					// blood small
    injector::MakeCALL(0x0055FECC, AddParticleHook1, true);					// blood small
    injector::MakeCALL(0x0056120F, AddParticleHook1, true);					// blood small
    injector::MakeCALL(0x00562CB4, AddParticleHook1, true);					// blood
    injector::MakeCALL(0x00563DD4, AddParticleHook1, true);					// blood
    injector::MakeCALL(0x00563F73, AddParticleHook1, true);					// boat splash

    addParticle2.fun = injector::MakeCALL(0x004BD72B, AddParticleHook2, true).get();	// car splash
    injector::MakeCALL(0x004BD8FF, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x004BDAD3, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x004BDC7D, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x004BDE56, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x004BDFBD, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x004BE11E, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x004BE25A, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x004BE504, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x004BE6CE, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x004BE898, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x004BEA44, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x004BEBF6, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x004BED32, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x004BEE68, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x004BEF79, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x004C87DF, AddParticleHook2, true);					// rain splash big grow
    injector::MakeCALL(0x004CCD69, AddParticleHook2, true);					// ped splash
    injector::MakeCALL(0x00530EF0, AddParticleHook2, true);					// rain splash big grow
    injector::MakeCALL(0x00531369, AddParticleHook2, true);					// ped splash
    injector::MakeCALL(0x00540744, AddParticleHook2, true);					// boat thrust jet
    injector::MakeCALL(0x00540855, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x00540892, AddParticleHook2, true);					// boat wake
    injector::MakeCALL(0x00540924, AddParticleHook2, true);					// boat splash
    injector::MakeCALL(0x00540A35, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x005414A8, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x005414D8, AddParticleHook2, true);					// boat splash
    injector::MakeCALL(0x00541737, AddParticleHook2, true);					// car splash
    injector::MakeCALL(0x00541767, AddParticleHook2, true);					// boat splash
}
